...
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* end_x_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* end_y_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* end_z_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* bone_1_x_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* bone_1_y_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* bone_1_z_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* bone_2_x_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* bone_2_y_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* bone_2_z_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* bone_3_x_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* bone_3_y_widget = nullptr;
nanogui::detail::FormWidget<float, std::integral_constant<bool, true>>* bone_3_z_widget = nullptr;
...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	bone_1_x_widget = gui_1->addVariable("Rotation X1", m_bone_animation->initial_bone_1_rotation[0]);
	bone_1_y_widget = gui_1->addVariable("Rotation Y1", m_bone_animation->initial_bone_1_rotation[1]);
	bone_1_z_widget = gui_1->addVariable("Rotation Z1", m_bone_animation->initial_bone_1_rotation[2]);

	bone_2_x_widget = gui_1->addVariable("Rotation X2", m_bone_animation->initial_bone_2_rotation[0]);
	bone_2_y_widget = gui_1->addVariable("Rotation Y2", m_bone_animation->initial_bone_2_rotation[1]);
	bone_2_z_widget = gui_1->addVariable("Rotation Z2", m_bone_animation->initial_bone_2_rotation[2]);

	bone_3_x_widget = gui_1->addVariable("Rotation X3", m_bone_animation->initial_bone_3_rotation[0]);
	bone_3_y_widget = gui_1->addVariable("Rotation Y3", m_bone_animation->initial_bone_3_rotation[1]);
	bone_3_z_widget = gui_1->addVariable("Rotation Z3", m_bone_animation->initial_bone_3_rotation[2]);

	gui_1->addButton("Reset Bone Rotation", []() {
		bone_1_x_widget->setValue(0);
		bone_1_y_widget->setValue(0);
		bone_1_z_widget->setValue(30);
		bone_2_x_widget->setValue(0);
		bone_2_y_widget->setValue(0);
		bone_2_z_widget->setValue(30);
		bone_3_x_widget->setValue(0);
		bone_3_y_widget->setValue(0);
		bone_3_z_widget->setValue(30);
		end_x_widget->setValue(m_bone_animation->initial_end_effector3.x);
		end_y_widget->setValue(m_bone_animation->initial_end_effector3.y);
		end_z_widget->setValue(m_bone_animation->initial_end_effector3.z);
		m_bone_animation->reset();
	});

	static auto bones_widget = gui_1->addVariable("Bones move", bool_move);

	static auto target_x_widget = gui_1->addVariable("Target X", m_bone_animation->target_position[0]);
	static auto target_y_widget = gui_1->addVariable("Target Y", m_bone_animation->target_position[1]);
	static auto target_z_widget = gui_1->addVariable("Target Z", m_bone_animation->target_position[2]);

	end_x_widget = gui_1->addVariable("End X", m_bone_animation->initial_end_effector3.x);
	end_y_widget = gui_1->addVariable("End Y", m_bone_animation->initial_end_effector3.y);
	end_z_widget = gui_1->addVariable("End Z", m_bone_animation->initial_end_effector3.z);
	...
	glfwSetMouseButtonCallback(window,
		...
		m_bone_animation->target_mat[3][0] = m_bone_animation->target_position[0];
		m_bone_animation->target_mat[3][1] = m_bone_animation->target_position[1];
		m_bone_animation->target_mat[3][2] = m_bone_animation->target_position[2];
	}
	);
	...
	glfwSetKeyCallback(window,
		...
		if (key >= 0 && key < 1024)
		{
			...
			{
				keys[key] = true;
				target_x_widget->setValue(m_bone_animation->target_position[0]);
				target_y_widget->setValue(m_bone_animation->target_position[1]);
				target_z_widget->setValue(m_bone_animation->target_position[2]);
			}
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	glm::mat4 bone_1_obj_mat;
	glm::mat4 bone_2_obj_mat;
	glm::mat4 bone_3_obj_mat;
	glm::mat4 target_obj_mat = m_bone_animation->get_target_mat();
	glm::mat4 root_bone_obj_mat = m_bone_animation->get_root_bone_mat();
	if (bool_move == 1) {


		end_x_widget->setValue(m_bone_animation->end_effector3.x);
		end_y_widget->setValue(m_bone_animation->end_effector3.y);
		end_z_widget->setValue(m_bone_animation->end_effector3.z);
		bone_1_obj_mat = m_bone_animation->JacobianRotation1(m_bone_animation->target_position, m_bone_animation->end_effector1, m_bone_animation->bone_1_rotation);
		bone_2_obj_mat = m_bone_animation->JacobianRotation2(m_bone_animation->end_effector1, m_bone_animation->end_effector2, m_bone_animation->bone_2_rotation);
		bone_3_obj_mat = m_bone_animation->JacobianRotation3(m_bone_animation->end_effector2, m_bone_animation->end_effector3, m_bone_animation->bone_3_rotation);
		bone_1_x_widget->setValue(m_bone_animation->total_bone_1.x);
		bone_1_y_widget->setValue(m_bone_animation->total_bone_1.y);
		bone_1_z_widget->setValue(m_bone_animation->total_bone_1.z);
		bone_2_x_widget->setValue(m_bone_animation->total_bone_2.x);
		bone_2_y_widget->setValue(m_bone_animation->total_bone_2.y);
		bone_2_z_widget->setValue(m_bone_animation->total_bone_2.z);
		bone_3_x_widget->setValue(m_bone_animation->total_bone_3.x);
		bone_3_y_widget->setValue(m_bone_animation->total_bone_3.y);
		bone_3_z_widget->setValue(m_bone_animation->total_bone_3.z);
	}
	else {
		endEff = m_bone_animation->end_effector1.x;

		bone_1_obj_mat = m_bone_animation->bone_1_mat;
		bone_2_obj_mat = m_bone_animation->bone_2_mat;
		bone_3_obj_mat = m_bone_animation->bone_3_mat;
	}
	...
	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bone_1_obj_mat));
	bone_obj->obj_color = m_bone_animation->colors[2];
	draw_object(shader, *bone_obj);


	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bone_2_obj_mat));
	bone_obj->obj_color = m_bone_animation->colors[3];
	draw_object(shader, *bone_obj);

	glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bone_3_obj_mat));
